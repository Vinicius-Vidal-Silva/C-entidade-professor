...
#include <windows.h> // Biblioteca padrão para funções do sistema operacional Windows
...
void AtribuirNotas(char *materia)
{
    ...
    while (LerArquivo(file, &aluno))
    {
        ...
        if (strcmp(aluno.nome, nome) == 0)
        {
            ...
            // Solicita ao usuário qual avaliação deseja alterar
            printf("Escolha qual avaliação deseja lançar/alterar a nota:\n1 - P1\n2 - P2\n");
            scanf("%d", &escolha);

            // Se o usuário escolher alterar a nota de P1
            if (escolha == 1)
            {
                printf("Digite a nova nota para P1: ");
                scanf("%f", &novaNota);  // Lê a nova nota
                aluno.p1 = novaNota;     // Atualiza a nota de P1 do aluno
            }
            // Se o usuário escolher alterar a nota de P2
            else if (escolha == 2)
            {
                printf("Digite a nova nota para P2: ");
                scanf("%f", &novaNota);  // Lê a nova nota
                aluno.p2 = novaNota;     // Atualiza a nota de P2 do aluno
            }
            // Se a escolha do usuário for inválida
            else
            {
...
void FechamentoMateria(char *materia)
{
    ...
    if (opcao == 1)
    {
        ...
        int encontrado = 0; // Variável para rastrear se o aluno foi encontrado
...
void AlterarUsuario(const char *nomeArquivo, const char *cpfAtual)
{
    ...
    // Lê o CPF do usuário que será alterado
    char cpf[12];
    printf("Digite o CPF do usuário a ser alterado: ");
    scanf("%11s", cpf);

    // Limpa o buffer de entrada após ler o CPF
    while (getchar() != '\n');
...
int main()
{
    ...
    while (1)
    {
        ...
        if (Login(nomeArquivo, cpf, senha, &usuarioEncontrado))
        {
            ...
            {
            case 1:
                Administrador(usuarioEncontrado);
                break;
            case 2:
